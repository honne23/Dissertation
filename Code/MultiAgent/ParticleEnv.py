...
def play(env, n_round, map_size, max_steps, handles, models, print_every, eps=1.0, render=False, train=False):
    ...
    while not done and step_ct < max_steps:
        ...
        for i in range(n_group):
            ...
            input_state = np.concatenate((state[i], former_act_prob[i]), axis=0)
            acts[i] = models[i].select_action(input_state, step_ct >= min_train)
        ...
        buffer = {
            ...
            'alives': alives[0], 'ids': ids[0], 'prob':former_act_prob[0]
        }
        ...
        if train:
            transition = [state[0], acts[0], rewards[0], next_state, done]
            models[0].memory.store(transition)
